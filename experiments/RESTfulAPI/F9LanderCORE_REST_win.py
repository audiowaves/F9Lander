...
# -------------------------------------------------- #
# just make the window position better suitable for Windows
...
class Simulation(object):
    ...
    def step(self, world_obj, simulation_array=[], command="9999"):   # "9999" is a placeholder
        ...
        # manage tick rates to prevent hanging up in Windows
        clock.tick(FRAMERATE)
        ...
        if self.display:
            for event in pygame.event.get():
                if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                    ...
                    # this exits correctly in Windows
                    sys.exit()
...
# rest
# app.run(debug=False)
app.run(threaded=False, debug=False, host=args.ip, port=args.port)
